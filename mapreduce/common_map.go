...
func doMap(
	...
	for i := 0; i < nReduce; i += 1 {
		...
		defer f.Close()
		outputs = append(outputs, json.NewEncoder(f))
	}

	// !!! Write kv pairs to intermediate files
	for _, kv := range keyValues {
		if err = outputs[ihash(kv.Key)%nReduce].Encode(&kv); err != nil {
			log.Fatal(err)
		}
	}
}
